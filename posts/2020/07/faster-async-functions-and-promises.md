...
먼저, V8은 함수를 resumable로 표시하는데, 이 것은 실행이 중지될 수 있으며 이후에 다시 재개될 수 있음을 의미한다. (`await`) 그리고 `implicit_promise`라는 것을 만든다. 이것은 `async`함수가 실행 될때 리턴되는 promise의 한종류이며, `async` 함수에 의해서 resolve 된다.

![async 함수와 V8엔진의 동작](https://v8.dev/_img/fast-async/await-under-the-hood.svg)

여기서 흥미로운 것을 하나 발견할 수 있는데, 바로 `await`이다. 먼저 `await`으로 넘겨진 값은 `promise`로 감싸진다. 그다음, 이렇게 감싸진 `promise`에 핸들러가 붙게되는데, 이는 promise 가 실행된 이후에 함수를 재개하는 역할을 한다. 그리고 async 함수가 멈추게 되면, `implicit_promise`를 호출자에게 리턴한다. `promise`가 실행되면, async 함수의 실행은 `promise`가 리턴한 `w`와 함께 재개 되며, `implicit_promise`는 `w`를 resolve 하게 된다.
...
그 다음 엔진은 `throwaway` 라고 불리우는 `promise`를 만든다. 이렇게 불리게 된 이유는 이것은 어디에도 체이닝 되지 않기 때문이다. 이는 완전히 엔진 내부 작동과 연관되어 있다. 이 일회성 promise는 비동기 기능을 재개하기 위한 적절한 핸들러와 함께 Promise에 바인딩 된다. 이 `performPromiseThen` 는 [Promise.prototype.then()](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/then) 구현에 필수적이다. 그리고 마지막으로, async 함수가 중지되고, 컨트롤이 호출자로 넘어간다.

![](https://v8.dev/_img/fast-async/await-step-2.svg)

호출자 내에서 실행이 지속되는데, 이는 결과적으로 호출 스택이 비어지게 된다. 그리고 자바스크립트 엔진은 마이크로 태스크를 실행하게 된다. 이는 이전에 스케쥴링 해두었던 [PromiseResolveThenableJob](https://tc39.es/ecma262/#sec-promiseresolvethenablejob) 을 실행하는데, 이는 새로운 [PromiseReactionJob](https://tc39.es/ecma262/#sec-promisereactionjob) `promise` 체인을 `await`올 넘겨받은 값에 연결한다. 그 다음, 엔진은 마이크로태스크 큐를 진행하고, 마이크로 테스크큐는 다음 메인 이벤트 루프가 실행되기 전까지 비워두게 된다.

![](https://v8.dev/_img/fast-async/await-step-3.svg)

다음은 [PromiseReactionJob](https://tc39.es/ecma262/#sec-promisereactionjob)인데, 이는 `awaiting`하고 있는 `promise`를 실행하는 job을 의미한다. 이 경우에는 42로,  -->
