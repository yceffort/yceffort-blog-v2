---
title: '자바스크립트 의존성 관리자(npm, yarn, pnpm)는 사실 제대로 의존성 관리를 못하고 있다.'
tags:
  - javascript
  - npm
  - yarn
...
이 글에서는 결국 의존성 관리자(이하 패키지 관리자라고 칭하겠다)의 근본적인 문제를 해결하고자 하는 것은 아니다. 대신, 하드드라이브의 블랙홀을 막고, 종속성을 관리할 수 있는 가이드를 제공하고자 한다.

많은 도구가 그 모든 패키지들이 있는 `node_modules`를 분석하고, 처리한다. 여기에 패키지가 설치될 수록 설치시간이 느려지게 된다. 하물며 일부 라이브러리에서 패키지 의존성이 심해지면, 이를 사용하는 전세계 모든 사용자들의 작업이 느려지는 결과를 낳게 될 것이다. 아래 방법을 적용하면 종속성 크기를 점차 줄여나가고, 안정적으로 유지할 수 있다.
