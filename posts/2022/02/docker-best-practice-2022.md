...
이미지를 빌드할때, 도커는 `Dockerfile` 내부에 지정되어 있는 순서대로 실행한다. 각 명령을 검토할 떄 도커는 새로운 (중복) 이미지를 만들지 않고 캐시에서 재사용할 수 있는 이미지를 찾는다.

캐시를 전혀 사용하지 않으려면 `--no-cache=true`를 사용할 수 있따. 그러나 도커가 캐시를 활용할 수 있도록 하기 위해서는, 일치하는 이미지를 찾을 수 있는 경우와 없는 경우에 대해 이해하는 것이 중요하다. 도커가 따르는 기본적인 규칙은 아래와 같다.
