...
V8에서, 숫자는 64비트 아키텍쳐를 기반으로 $$-2^{31}$$ 에서 $$2^{31}-1$$ 까지 구성되어 있다. 그리고 v8에서는 이를 `smi` 라고 부르는데, 이들은 내부적으로 최적화가 되어 있어 포인터에서 추가적인 스토리지를 할당할 필요 없이 포인터 내부에서 인코딩할 수 있다. 이는 V8만의 특징은 아니고, 다른 언어에서도 발견할 수 있다.

기술적으로, `smi`는 변수가 선언되는 방식에 따라서 힙에 추가적인 메모리를 할당할 필요가 없어 스택에 존재하는 경우가 있을 수도 있다.

- `const a = 1`은 스택에 있을수도 있다.
- `var a = 1`의 경우에느, 글로벌 변수의 속성을 가지게 되므로 (전영객체) 메모리의 고정 위치인 힙에 존재한다.

또한 스크립트의 나머지 작업이나 런타임 환경에 따라 달라질 수도 있다. 그리고 이 최적화 컴파일러는 가능한 많은 포인터를 레지스터에 보관하고, 이 레지스터가 더이상 자리가 없는 고갈된 상황에서만 스택으로 보낸다.
...
위코드에서, `smiNumber`인 123은 같은 메모리 포인터를 가리키고 있는 것을 볼 수 있다. `123`에 대해 동일한 비트와이즈 패턴을 가지고 있다.

`heap number`의 경우, 이 둘은 서로 다른 포인터를 가리키고 있는데, 이는 mutable한 `heapNumber`로 취급 되어 재사용되지 않기 때문이다.

정리하자면

- `smi`의 경우, 이들은 invalid한 포인터로 인식되고, 이들은 아무것도 가리키고 있지 않기 때문에 '재사용' 이라는 개념은 `smi`에 한해서 유효하지 않다.
- `smi`가 아닌 `heapnumber`는
  - 객체의 속성으로 지정되면, mutable한 `heapnumber`가 되어 새로운 `heapnumber`를 매번 할당하는 것이아니라 값을 업데이트 하는 방식으로 관리된다.
  - 다른 경우에는 재사용 가능하지만, 추가 적인 성능 오버헤드가 발생하지 않는 경우에만 재사용이 가능하다. 예를 들어, $$3 + 0.14$$ 나 $$\frac{314}{100}$$ 과 같은 연산은 3.14라는 `heapnumber`가 이미 존재하는지 확인할 필요가 없기 때문에 (이미 연산했으므로) 각각의 다른 `heapnumber`가 할당된다.
