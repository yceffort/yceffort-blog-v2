...
- `useEffect`안에 넣어서 이러한 변화를 추적하고ㄱ자 하는 경우
...
그렇다면, 모든 것을 `memo`로 한다음, 잘못된 `memo`를 했을 때 비용은 얼마나 드는가 생각해봐야 한다. 만약 `memo` 컴포넌트를 잘 못 썼다면, 매 리렌덜이 시에 여기에서 props에 대한 얕은 비교를 수행할 것이다. 그리고 이에 따른 절차는 아래와 같을 것이다.
...
만약 `memo`가 더 cpu에 무리가 가는 일이라면 어떨까? 경험상 그렇지 않은 것 같다. `memo`로 인한 문제가 프로파일에 뜨는 것을 본 적은 거의없지만, 렌더링이 cpu 시간을 소모하는 것음 ㅐ우 일반적이다. 일반적인 문제는, 너무 많은 컴포넌트가 한번에 마운팅 되는 등의 문제다.
...
새 객체나 배열을 만들떄도 마찬가지다. 이를 `useMemo`로 감싸지 않으면, props를 받는 모든 컴포넌트를 리렌더링할 것이다.
