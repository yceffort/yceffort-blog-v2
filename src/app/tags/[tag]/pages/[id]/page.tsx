...
export async function generateStaticParams() {
    ...
    allTags.forEach(({tag}) => {
        const tagsCount: number = posts.filter((post) => post.frontMatter.tags.find((t) => t === tag)).length

        ;[...new Array(Math.round(tagsCount / DEFAULT_NUMBER_OF_POSTS)).keys()].forEach((i) => {
...
export default async function Page(props: {params: Promise<{tag: string; id: string}>}) {
    ...
    const postsWithTag = allPosts.filter((post) => post.frontMatter.tags.find((t) => t === tag))

    if (isNaN(pageNo) || pageNo > Math.ceil(postsWithTag.length / DEFAULT_NUMBER_OF_POSTS) || pageNo < 1) {
        return notFound()
    }
    ...
    const hasNextPage = Math.ceil(postsWithTag.length / DEFAULT_NUMBER_OF_POSTS) > pageNo
