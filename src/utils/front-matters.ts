...
function getFilesRecursively(path: string) {
  ...
  const getFiles = (path: string) =>
    readdirSync(path).map(name => join(path, name)).filter((path: string) => statSync(path).isFile());
  ...
  const files: string[] = dirs.map((dir) => getFilesRecursively(dir)).reduce((a, b) => a.concat(b), [])
